...
into activevols
from eventattendees
where eventtype in ('Voter Reg', 'Canvass', 'Phone Bank', 'Vol Recruitment','Data Entry','Signature Gathering','GOTV Event')
and finalsitch=7
...
where eventtype in (/*'Voter Reg', 'Canvass', 'Phone Bank', 'Vol Recruitment','Signature Gathering','1-on-1 Meeting',*/'GOTV Event')
;

drop table sroberts.eventsummary;
select * into sroberts.eventsummary 
from (
select timeperiod
, Region /*FO 'Statewide'*/ turf
--,eventtype
,case when left(rolename,9)='Canvasser' then 'Canvass'
        when substring(rolename,15,6)='Dialer' then 'Dialer' 
        when left(rolename,12)='Phone banker' then 'Phone Bank' else 'Lead' end eventtype
...
from eventattendees
where eventtype in ('Voter Reg', 'Canvass', 'Phone Bank', 'Vol Recruitment','GOTV Event')
...
,coalesce(ut.cnvrtype,'Vol') cnvrtype
,count(distinct case when contacttypeid in (1,19) then ch.contactscontactid::varchar when datecanvassed::date>'2014-10-30' then 'P3'||addressid::varchar when datecanvassed::date>'2014-10-21' then 'P2'||addressid::varchar when datecanvassed::date>'2014-10-12' then 'P1'||addressid::varchar else addressid::varchar end) DVCAttempts
...
where (case ch.datecanvassed --corrects for central time if not manual(hence 'midnight') entry
        when ch.datecanvassed::date then ch.datecanvassed
        else ch.datecanvassed-interval '1 Hour' end)>=(current_date-1-extract(dow from current_date+1)::int%7)
...
insert into myvcontacthistory
select case 
        when ch.datecanvassed::date>='2014-11-01' then 'Phase 3' 
        when ch.datecanvassed::date>='2014-10-20' then 'Phase 2'
        when ch.datecanvassed::date>='2014-10-10' then 'Phase 1'
        end timeperiod
, 'Statewide' turf--, coalesce(attr.foname,'Unturfed') 
,case contacttypeid when 1 then 'Phone Bank' when 19 then 'Dialer' else 'Canvass' end eventtype
,'Unicorns' cnvrtype --in the gdoc, it uses '<>unicorns' in an if() statement for sumifs ofr the general report
,count(distinct case when contacttypeid in (1,19) then ch.contactscontactid::varchar when datecanvassed::date>'2014-10-30' then 'P3'||addressid::varchar when datecanvassed::date>'2014-10-21' then 'P2'||addressid::varchar when datecanvassed::date>'2014-10-12' then 'P1'||addressid::varchar else addressid::varchar end) DVCAttempts
,sum(case when resultid=14 then 1 end) DVCContacts
from c2014_ar_coord_vansync.contacthistoryexport ch
left join analytics_ar.vanid_to_personid vtp using(vanid,state) 
...
where (case ch.datecanvassed --corrects for central time if not manual(hence 'midnight') entry
        when ch.datecanvassed::date then ch.datecanvassed
        else ch.datecanvassed-interval '1 Hour' end)>='2014-10-10'--(current_date-1-extract(dow from current_date+1)::int%7)
and contacttypeid in (1,2,19)
and ch.committeeid=45240--Our's
group by 1,2,3,4
;
